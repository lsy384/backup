...
def run_step_5_2_calibration(result, ease_lat, ease_lon, output_dir):
    ...
    hr_use = torch.full_like(znd_M09_surf, 0.467)
    ...
    def calc_rmse_robust(sim, obs, polar_type=None):
        """
        使用【物理阈值 + 稳健统计MAD】双重过滤异常值并计算 RMSE
        """
        # 确保转换为 Tensor
        sim = torch.as_tensor(sim).detach().cpu()
        obs = torch.as_tensor(obs).detach().cpu()
        err = sim - obs
        
        # ---- 第一重：物理常规边界过滤（针对图中的 2016-07 观测错误点） ----
        if polar_type == 'H':
            phys_mask = obs > 250.0  # H 极化正常观测绝不可能低于 250K
        elif polar_type == 'V':
            phys_mask = obs > 270.0  # V 极化正常观测绝不可能低于 270K
        else:
            phys_mask = torch.ones_like(obs, dtype=torch.bool)
            
        # ---- 第二重：稳健统计学过滤（使用 MAD 代替标准差，防止标准差被异常值拉高） ----
        median_err = torch.median(err)
        mad = torch.median(torch.abs(err - median_err))
        robust_std = 1.4826 * mad  # 统计学上，1.4826*MAD 约等于不受异常值污染的标准差
        
        # 允许残差在稳健中位数的 3 倍偏差以内
        mad_mask = torch.abs(err - median_err) < 3 * robust_std
        
        # 合并两层掩码
        final_mask = phys_mask & mad_mask
        
        # 计算过滤后的有效 RMSE
        if final_mask.sum() == 0:
            return float('nan')
        return torch.sqrt(torch.mean(err[final_mask] ** 2)).item()
    # 传入 'H' 和 'V' 自动切掉 2016-07 的断崖错误观测点
    rtm_obs_h = calc_rmse_robust(tb_h_sim, obs_tb_h, polar_type='H')
    rtm_obs_v = calc_rmse_robust(tb_v_sim, obs_tb_v, polar_type='V')
    rtm_obs_avg = (rtm_obs_h + rtm_obs_v) / 2.0
    # 如果 CoLM 模拟数据没有这种突变错误，直接用 MAD 统计过滤即可
    rtm_colm_h = calc_rmse_robust(tb_h_sim, t_brt_smap_h)
    rtm_colm_v = calc_rmse_robust(tb_v_sim, t_brt_smap_v)
    rtm_colm_avg = (rtm_colm_h + rtm_colm_v) / 2.0
    print(f"【完美剔除异常后】RTM 模拟 与 卫星观测 误差:")
    print(f"  -> H极化 RMSE: {rtm_obs_h:.4f} K | V极化 RMSE: {rtm_obs_v:.4f} K | 两极化平均 : {rtm_obs_avg:.4f} K")
    ...
    tb_h_sim_exp = np.repeat(tb_h_sim.detach().cpu().numpy(), patch_dim)
    tb_v_sim_exp = np.repeat(tb_v_sim.detach().cpu().numpy(), patch_dim)
    date_exp = np.repeat(obs['date'], patch_dim)
    time_index_exp = np.repeat(obs['time_index'], patch_dim)
    t_eff_array = rtm_model.t_eff.detach().cpu().numpy()
    # 将 t_h2osoi 转换为 numpy 数组，形状为 (N, 10)
    h2osoi_array = t_t_soi.detach().cpu().numpy()
    # 动态生成 10 层土壤水分的列字典
    h2osoi_dict = {f"t_soi_layer_{i+1}": h2osoi_array[:, i] for i in range(10)}
    fs_array = rtm_model.wilhet_fs.detach().cpu().numpy()
    fs_dict = {f"fs_layer_{i+1}": fs_array[:, i] for i in range(10)}
    ...
    df_out = pd.DataFrame({
        ...
        "tb_h_sim_exp": tb_h_sim_exp,
        "tb_v_sim_exp": tb_v_sim_exp,
    ...
    # ----------------- 绘图代码补充 -----------------
    # 确保所有数据都转为普通的 numpy 数组
    obs_h_raw = obs['tb_h']
    obs_v_raw = obs['tb_v']
    
    # 如果 tb_h_sim 是 PyTorch 张量，先转到 CPU 并转为 numpy
    if hasattr(tb_h_sim, 'detach'):
        sim_h_raw = tb_h_sim.detach().cpu().numpy()
        sim_v_raw = tb_v_sim.detach().cpu().numpy()
    else:
        sim_h_raw = tb_h_sim
        sim_v_raw = tb_v_sim

    # ★ 核心修改：将 X 轴指定为日期，并转换为 datetime 格式方便绘图自适应
    x_axis = pd.to_datetime(obs['date'])

    # ----------------- 开始绘制 2*1 对比图 -----------------
    sns.set_theme(style="whitegrid") # 美化界面
    fig, axes = plt.subplots(2, 1, figsize=(12, 10), sharex=True)

    # 1. H 极化亮温对比 (未经过 repeat 扩充)
    axes[0].plot(x_axis, obs_h_raw, label='Observed (H)', color='#1f77b4', marker='o', markersize=4, linewidth=1.5, alpha=0.8)
    axes[0].plot(x_axis, sim_h_raw, label='Simulated (H)', color='#ff7f0e', marker='x', markersize=4, linestyle='--', linewidth=1.5)
    axes[0].set_ylabel('Brightness Temperature (K)', fontsize=12)
    axes[0].set_title('H-Polarization: Raw Observation vs Simulation (No Repeat)', fontsize=14, fontweight='bold')
    axes[0].legend(loc='upper right', frameon=True)
    axes[0].grid(True, linestyle=':', alpha=0.6)

    # 2. V 极化亮温对比 (未经过 repeat 扩充)
    axes[1].plot(x_axis, obs_v_raw, label='Observed (V)', color='#2ca02c', marker='o', markersize=4, linewidth=1.5, alpha=0.8)
    axes[1].plot(x_axis, sim_v_raw, label='Simulated (V)', color='#d62728', marker='x', markersize=4, linestyle='--', linewidth=1.5)
    axes[1].set_xlabel('Date', fontsize=12)  # 修改标签为 Date
    axes[1].set_ylabel('Brightness Temperature (K)', fontsize=12)
    axes[1].set_title('V-Polarization: Raw Observation vs Simulation (No Repeat)', fontsize=14, fontweight='bold')
    axes[1].legend(loc='upper right', frameon=True)
    axes[1].grid(True, linestyle=':', alpha=0.6)
    # ★ 优化：旋转底部子图的日期标签，防止太密时重叠
    plt.xticks(rotation=45, ha='right')
    # 自动优化布局
    plt.tight_layout()
    # 确保输出目录存在
    plt.savefig(os.path.join(output_dir, f'lat_{ease_lat}_lon_{ease_lon}_tb_comparison_h_v.png'), dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    for grid_id, position in enumerate(positions[0:5]):  
