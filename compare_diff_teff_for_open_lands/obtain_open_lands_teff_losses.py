...
def run_step_5_2_calibration(result, ease_lat, ease_lon, output_dir):
    ...
    # === 新增：直接从模型属性读取有效温度并计算指标 ===
    t_wig = rtm_model.t_eff_wigneron.detach().cpu().numpy()
    t_hol = rtm_model.t_eff_holmes.detach().cpu().numpy()
    t_wil = rtm_model.t_eff_wilheit.detach().cpu().numpy()

    # 以 wilheit 为基准计算 Bias 和 RMSE
    bias_wig = np.mean(t_wig - t_wil)
    rmse_wig = np.sqrt(np.mean((t_wig - t_wil)**2))

    bias_hol = np.mean(t_hol - t_wil)
    rmse_hol = np.sqrt(np.mean((t_hol - t_wil)**2))

    metrics = {
        'ease_lon': ease_lon,
        'ease_lat': ease_lat,
        'Holmes2006_RMSE': rmse_hol,
        'Holmes2006_Bias': bias_hol,
        'Wigneron2001_RMSE': rmse_wig,
        'Wigneron2001_Bias': bias_wig
    }
...
if __name__ == "__main__":
    ...
    print("正在获取所有网格坐标...")
    df_patch = pd.read_csv(patch_map_file)
    # 0. 提取唯一值并先按照 ease_lat 排序，确保基础顺序正确
    df_unique = df_patch[['ease_lat', 'ease_lon']].drop_duplicates().sort_values('ease_lat')
    # 【选法 A】等间隔抽样：每 10 个取 1 个（严格等距，最符合“均匀”的字面意思，且天然按 lat 有序）
    unique_grids = df_unique.iloc[::10].values
    print('len(unique_grids)',len(unique_grids))
    # === 新增：循环开始前，先创建好 CSV 文件并写入表头 ===
    summary_csv_path = os.path.join("teff_comparison_summary.csv")
    cols = ['ease_lon', 'ease_lat', 'Holmes2006_RMSE', 'Holmes2006_Bias', 'Wigneron2001_RMSE', 'Wigneron2001_Bias']
    # 建立一个空的 DataFrame 并输出，这一步会覆盖同名的旧文件并写入表头
    pd.DataFrame(columns=cols).to_csv(summary_csv_path, index=False)
    print(f"📄 已初始化结果记录文件: {summary_csv_path}")
    
    for ease_lat, ease_lon in unique_grids:
        target_csv = os.path.join(csv_dir, f"lat_{ease_lat}_lon_{ease_lon}_SMAP_TB.csv")
        
        # 稳健性检查：如果没有这个 CSV 就跳过
        if not os.path.exists(target_csv):
            continue
            
        print(f"\n正在处理网格: 经度={ease_lon}, 纬度={ease_lat}")
        try:
            df_obs = pd.read_csv(target_csv)
            result = process_single_grid(df_nc_index, df_obs, patch_map_file, nc_dir) 
            
            # 接收刚才函数中 return 的指标字典
            metrics = run_step_5_2_calibration(result, ease_lat, ease_lon, output_dir)
            
            # === 新增：动态追加写入 CSV ===
            if metrics is not None:
                # 转换为 DataFrame 单行
                df_row = pd.DataFrame([metrics])[cols] # 确保列顺序一致
                # mode='a' 表示追加，header=False 表示不再重复写表头
                df_row.to_csv(summary_csv_path, mode='a', header=False, index=False)
                print(f"👉 当前网格指标已成功追加至 CSV")
                
        except Exception as e:
            print(f"⚠️ 处理网格 (经度: {ease_lon}, 纬度: {ease_lat}) 时抛出异常: {e}")
            continue

    print(f"\n✅ 全部遍历完成！所有有效数据已保存在: {summary_csv_path}")
